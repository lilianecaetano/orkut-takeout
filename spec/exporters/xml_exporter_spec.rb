...
describe XMLExporter do
	...
	let(:beginner_friends_hash) {
		JSON.parse(%Q([{"_id":"56ab7fc22e481306042c151d","user":{"_id":"56ab7ee72e481306042c1518","name":"QA Couse User 1","email":"qacourseuser1@avenuecode.com"}},{"_id":"56ab7fd22e481306042c151e","user":{"_id":"56ab7ef12e481306042c1519","name":"QA Couse User 2","email":"qacourseuser2@avenuecode.com"}}]))
	}

	context "should export my friends in XML format" do
		it "should say my social type is FRIENDLY" do
			xml_exported = exporter.export_friends(friend_hash,current_user_hash)
		...
		it "should say my social type is BEGINNER" do
			xml_exported = exporter.export_friends(beginner_friends_hash,current_user_hash)
			xml_exported = Hash.from_xml(xml_exported)
			expect(xml_exported["user"]["socialType"]).to eq("BEGINNER")
		end

			it "should say my social type is PARTY_ANIMAL" do
			xml_exported = exporter.export_friends(party_animal_friends,current_user_hash)
			xml_exported = Hash.from_xml(xml_exported)
			expect(xml_exported["user"]["socialType"]).to eq("PARTY_ANIMAL")
		end
	end
